...
class SymmetricCipherContext:
    def __init__(self,
        ...
        self._executor = ThreadPoolExecutor(max_workers=max_workers or (os.cpu_count() * 2))
    ...
    async def encrypt_file(self, input_path: str, output_path: str, chunk_size: int = 1024 * 1024) -> None:
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, self._encrypt_file_sync, input_path, output_path, chunk_size)
    ...
    async def decrypt_file(self, input_path: str, output_path: str, chunk_size: int = 1024 * 1024) -> None:
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, self._decrypt_file_sync, input_path, output_path, chunk_size)
        print(f"File decrypted successfully: {input_path} -> {output_path}")

    def _encrypt_file_sync(self, input_path: str, output_path: str, chunk_size: int) -> None:
    ...
    def _decrypt_file_sync(self, input_path: str, output_path: str, chunk_size: int) -> None:
        with open(input_path, "rb") as fin, open(output_path, "wb") as fout:
            match self.mode:
                case CipherMode.ECB:
    ...
    def _decrypt_cbc(self, data: bytes) -> bytes:
        ...
        decrypted_blocks = list(self._executor.map(self._worker_decrypt, ciphertext_blocks))
    ...
    def _encrypt_pcbc(self, data: bytes) -> bytes:
        ...
        for P_i in split_blocks(padded, bs):
            C_i = self.primitive.encrypt_block(xor_bytes(P_i, xor_bytes(prev_plain, prev_cipher)))
            ciphertext.append(C_i)
    ...
    def _decrypt_random_delta_file(self, fin, fout, chunk_size):
        ...
        while True:
            ...
            if full:
                pairs = [(full[i:i + bs], full[i + bs:i + bs * 2])
    ...
    def _decrypt_random_delta(self, data: bytes) -> bytes:
        ...
        xor_blocks = [xor_bytes(c_block, delta) for delta, c_block in delta_cipher_pairs]
